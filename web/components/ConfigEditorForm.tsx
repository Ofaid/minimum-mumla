...
import {
  ChevronDown, ChevronUp, Eye, EyeOff, Info, Plus, Settings2, Trash2
} from 'lucide-react';
...
type Channel = {
  ...
  presetKey?: string; access: { mode: 'none' | 'public' | 'protected'; token?: string; tokenRef?: string };
};
...
export function ConfigEditorForm({ config, model, onChange }: { config: MinimumConfig; model: ModelProfile; onChange: (config: MinimumConfig) => void }) {
  ...
  const tabs: Array<[Tab, string]> = [
    ['general', 'General'], ['connections', 'Servers'], ['channels', 'Channels'],
  ...
  return <div className="config-workbench">
    ...
    <div className="config-workbench-body">
      ...
      {tab === 'channels' && <Section title="Channel list" description="Give each channel a short alias. The full path is kept for the Mumble target." action={<button type="button" className="quiet-button" onClick={() => { const id = uniqueId('channel', channels.map((channel) => channel.id)); const firstConnection = Object.keys(connections)[0] || 'server-1'; const next = [...channels, { id, label: 'New channel', alias: '', connectionId: firstConnection, path: '/', access: { mode: 'none' as const } }]; update({ channels: next }); }}><Plus size={15} /> Add channel</button>}><div className="default-channel-picker"><label className="field"><span>Start with channel</span><select value={value.radio?.defaultChannel || ''} onChange={(event) => updateNested('radio', { defaultChannel: event.target.value })}>{channels.map((channel) => <option key={channel.id} value={channel.id}>{channel.alias || channel.label} ({channel.id})</option>)}</select></label><small className="field-help">The last selected channel is restored after restart or reconnect.</small></div><div className="repeat-list">{channels.map((channel, index) => <div className="repeat-card channel-card" key={`${channel.id}-${index}`}><div className="repeat-card-heading"><div><span className="eyebrow">CHANNEL {index + 1}</span><strong>{channel.alias || channel.label || channel.id}</strong></div><div className="reorder-actions"><button type="button" className="icon-button" disabled={index === 0} onClick={() => { const next = [...channels]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; update({ channels: next }); }} title="Move up" aria-label="Move channel up"><ChevronUp size={15} /></button><button type="button" className="icon-button" disabled={index === channels.length - 1} onClick={() => { const next = [...channels]; [next[index], next[index + 1]] = [next[index + 1], next[index]]; update({ channels: next }); }} title="Move down" aria-label="Move channel down"><ChevronDown size={15} /></button><button type="button" className="icon-button danger-icon" disabled={channels.length <= 1} onClick={() => removeChannel(index)} title="Remove channel" aria-label="Remove channel"><Trash2 size={15} /></button></div></div><div className="form-grid compact-grid"><Field label="Channel alias" value={channel.alias || ''} onChange={(alias) => updateChannel(index, { alias })} placeholder="Dispatch" help="Short name displayed prominently on Minimum." /><Field label="Channel label" value={channel.label || ''} onChange={(label) => updateChannel(index, { label })} placeholder="Operations" /><label className="field"><span>Server</span><select value={channel.connectionId} onChange={(event) => updateChannel(index, { connectionId: event.target.value })}>{Object.keys(connections).map((connectionId) => <option key={connectionId} value={connectionId}>{connections[connectionId].name || connectionId}</option>)}</select></label><Field label="Full channel path" value={channel.path || '/'} onChange={(path) => updateChannel(index, { path: path.startsWith('/') ? path : `/${path}` })} placeholder="/PUBLIC/MAIN" /><Field label="Preset key (optional)" value={channel.presetKey || ''} onChange={(presetKey) => updateChannel(index, { presetKey: presetKey.toUpperCase() })} placeholder="P1" help="Physical channel shortcut, such as P1 or P2." /><label className="field"><span>Access</span><select value={channel.access?.mode || 'none'} onChange={(event) => { const mode = event.target.value as Channel['access']['mode']; updateChannel(index, { access: mode === 'none' ? { mode } : { ...channel.access, mode } }); }}>{['none', 'public', 'protected'].map((mode) => <option key={mode} value={mode}>{mode === 'none' ? 'No token' : mode === 'public' ? 'Token in config' : 'Protected token reference'}</option>)}</select></label>{channel.access?.mode === 'public' && <SecretField label="Channel access token" value={channel.access.token || ''} visible={Boolean(visibleSecrets[`channel:${channel.id}`])} onToggle={() => setVisibleSecrets((state) => ({ ...state, [`channel:${channel.id}`]: !state[`channel:${channel.id}`] }))} onChange={(token) => updateChannel(index, { access: { ...channel.access, mode: 'public', token } })} help="Stored privately and sent only to the device." />}{channel.access?.mode === 'protected' && <Field label="Token reference" value={channel.access.tokenRef || ''} onChange={(tokenRef) => updateChannel(index, { access: { ...channel.access, mode: 'protected', tokenRef } })} help="Reference to a provisioned secret on the device." />}</div></div>)}</div></Section>}
