...
| RYKS screen-off PTT | PHYSICAL OEM EDGE PASS / LIVE TX VISUAL OPEN | From verified `Asleep` / display OFF, a physical hold emitted `com.zello.ptt.down/up` about 3.9 s apart and woke RadioShell. The receiver now dispatches directly to the running foreground service without queuing; a supervised capture of the TX state during the hold remains open |
