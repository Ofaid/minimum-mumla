...
public class AudioHandler extends HumlaNetworkListener implements AudioInput.AudioInputListener {
    ...
    public synchronized void shutdown() {
        RuntimeException failure = CleanupRunner.runAll(
                () -> {
                    if (mHalfDuplex) {
                        // A disconnect can bypass the normal talking=false transition.
        ...
        mInitialized = false;
        mBluetoothOn = false;
        RuntimeException listenerFailure = CleanupRunner.runAll(
                () -> mEncodeListener.onTalkingStateChanged(false));
        if (failure == null) {
            failure = listenerFailure;
        } else if (listenerFailure != null) {
            failure.addSuppressed(listenerFailure);
        }
        if (failure != null) {
            Log.e(TAG, "Audio teardown completed with a resource failure", failure);
        }
    }
