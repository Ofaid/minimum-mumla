...
public class HumlaService extends Service implements IHumlaService, IHumlaSession, HumlaConnection.HumlaConnectionListener, HumlaLogger, BluetoothScoReceiver.Listener {
        ...
        RuntimeException failure = CleanupRunner.runAll(
                ...
                () -> {
                    ...
                    try {
                        ...
                        // Also cover partial startup and the already-unregistered path.
                    }
                },
                () -> {
                    if (mConnection != null) {
                ...
                () -> {
                    try {
                        ...
                        // A partial service startup may not have registered the receiver.
                    }
                });
        if (failure != null) {
            Log.e(TAG, "Service teardown completed with a resource failure", failure);
        }
        super.onDestroy();
    }
    ...
    protected void connect() {
        if (mDestroyed || mConnectionState == ConnectionState.CONNECTING) {
            return;
        }
...
//            auth.addCeltVersions(Constants.CELT_11_VERSION);
    ...
    public void onConnectionSynchronized() {
        ...
        Log.v(TAG, "Connected");
        // The service owns this lock for the full connected audio lifecycle; teardown is idempotent.
        mWakeLock.acquire();
    ...
    private void cleanupConnectionResources() {
        ...
        RuntimeException failure = CleanupRunner.runAll(
                () -> {
                    if (connection != null && audioHandler != null) {
        ...
        mModelHandler = null;
        mVoiceTargetId = 0;
        if (mWhisperTargetList != null) {
            mWhisperTargetList.clear();
        }
        if (failure != null) {
            Log.e(TAG, "Connection teardown completed with a resource failure", failure);
        }
    }
